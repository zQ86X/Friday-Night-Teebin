...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	public function new(curStage)
	{
		...
		if (PlayState.determinedChartType == "FNF")
		{
			...
			curStage = switch (CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()))
			{
		...
		switch (curStage)
		{
			...
			case 'teeb':
			{
				...
				var bgSize = 1.25;
				var bgDiv = bgSize * bgSize;

				var piPow = Math.PI * Math.PI;
				var bg:FNFSprite = new FNFSprite(stageX / bgDiv, stageY / bgDiv).loadGraphic(Paths.image('backgrounds/$curStage/Background'));

				bg.setGraphicSize(Std.int(bg.width * bgSize));
				bg.updateHitbox();
				...
				var stageFront:FNFSprite = new FNFSprite(stageX, stageY).loadGraphic(Paths.image('backgrounds/$curStage/Foreground'));
				...
				crown = new FNFSprite(-50, -stageY / piPow / crownSize).loadGraphic(Paths.image('backgrounds/$curStage/Crown'));

				crown.setGraphicSize(Std.int(crown.width * crownSize));
				crown.updateHitbox();
				crown.antialiasing = FlxG.save.data.antialiasing;
				crown.scrollFactor.set(1, 1);
				crown.active = false;
			...
			case 'swagster' | 'jcom':
			{
				...
				var bg:FNFSprite = new FNFSprite(stageX, stageY).loadGraphic(Paths.image('backgrounds/$curStage/Background'));
