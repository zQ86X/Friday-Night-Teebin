...
class MenuCharacter extends FlxSprite
{
	...
	var curCharacterMap:Map<String, Array<Dynamic>> = [
		...
		'bf' => ["BF idle dance white", 24, true, .9, 100, 100],
		'bfConfirm' => ['BF HEY!!', 24, false, .9, 100, 100],
		'gf' => ["GF Dancing Beat WHITE", 24, true, 1, 100, 100],
		'dad' => ["Dad idle dance BLACK LINE", 24, true, .5, 0, 0],
		'spooky' => ["spooky dance idle BLACK LINES", 24, true, .5, 0, 90],
		'pico' => ["Pico Idle Dance", 24, true, .5, 0, 100],
	...
	public function createCharacter(newCharacter:String, canChange:Bool = false)
	{
		...
		if (assortedValues != null)
		{
			if (!visible)
				visible = true;
			trace(animation.getByName(assortedValues[0]));
			// animation
