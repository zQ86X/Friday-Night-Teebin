...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function updateDialog(force:Bool = false)
	{
		...
		if (pageData.speed != null)
			alphabetText.textSpeed = .045 / pageData.speed;
		else
			alphabetText.textSpeed = .045;
	...
	public function updatePortrait(force:Bool = false)
	{
		...
		if (curCharacter != newChar || force)
		{
			if (newChar != null)
			{
				...
				// im a dumbass and forgot to flip teeb
				switch (curCharacter)
				{
					case 'teeb': portrait.flipX = !newFlip;
					default: portrait.flipX = newFlip;
				}
